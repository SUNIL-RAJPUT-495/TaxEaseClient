...
import { Calculator, Check, ArrowRight, Loader2 } from "lucide-react";
...
const TaxPlanning = () => {
  ...
  const getdata = async () => {
    try {
      ...
      console.log("Tax Planning Data:", res.data);
              ...
              {plans.length > 0 ? (
                plans.map((plan, index) => (
                  <div
                    ...
                    className={`relative flex flex-col bg-white rounded-xl transition-all duration-300 animate-fade-up overflow-hidden ${
                      plan.isPopular 
                        ...
                        : "border border-slate-200 hover:border-emerald-300 hover:shadow-lg"
                    }`}
                  ...
                  >
                    ...
                    {plan.isPopular && (
                      <div className="absolute top-0 left-1/2 -translate-x-1/2 bg-emerald-500 text-white text-xs font-bold px-3 py-1 rounded-b-lg shadow-sm tracking-wide uppercase">
                        Most Popular
                      </div>
                    )}
